...
本次验证：图像 -> YOLOv8检测 + DPT相对深度 -> 物体类别、二维位置和相对深度 -> 环境记忆
后续扩展：加入SAM精细分割、相机标定和SLAM -> 赛博宠物的三维环境记忆
```

在完成多张图片检测后，项目还会把房间作为节点、已确认的通行关系作为边，构建一个简单的房间拓扑图。这样赛博宠物可以优先查询已记忆的房间路径，只有路线未知或环境发生变化时才重新进行视觉观察和规划。

### 为什么从二维检测开始

- YOLOv8可以直接下载轻量模型，适合作为家庭环境感知的轻量级起点。
- 输出的类别和位置，能够验证赛博宠物“先看见并记录家中物体”的基本链路。
- 官方文档指出分割和深度需要互补；本项目已在检测框基础上加入 DPT 单目深度估计，为每个物体补充相对深度。
- 当前深度是单目模型输出的相对值，不是经过相机标定的米制距离；SAM 精细分割和统一空间坐标仍是后续升级方向。

这使本项目同时具备两部分价值：**有实际运行结果的最小实践**，以及**对官方文档核心观点的验证和延伸**。

## 🧭 从环境记忆到路径复用

运行以下命令可以把两个已观察房间记录为一条路线，并查询这条路线：

```bash
python task3_home_memory.py --route sample_room,sample_bathroom --from-room sample_room --to-room sample_bathroom
```

输出：

```text
Remembered route: sample_room -> sample_bathroom
```

这里的路径是房间级拓扑路径，不是带真实米制坐标的机器人轨迹。结合当前物体相对深度后，系统可以进一步判断目标的大致远近；后续还需要相机标定、SLAM 和导航网格，才能将房间级路径细化为可执行路线。

## 🤖 记忆驱动的行为反馈

环境记忆还可以作为行为决策的输入。运行以下命令可以模拟赛博宠物根据已识别物体做出反馈：

```bash
python task3_home_memory.py --simulate-behavior
```

当前示例规则如下：

- 识别到 `bed`：模拟前往附近并进行问候。
- 识别到 `toilet` 或 `sink`：模拟后退一步并提醒注意。
- 识别到其他物体：保持观察。

行为函数当前只记录和打印计划动作，真实部署时可以在同一接口中调用机器人底盘、扬声器或其他执行器驱动。
